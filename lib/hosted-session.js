 ...
 * @param [forceInitialize] force the script to re-initialize (rewriting iframe)
 *
...
const getHostedSession = ({
  ...
  return getScript(merchantId)
    .then(() => {
      return configureSession(paymentSession, config, forceInitialize)
    })
}
 ...
 * @param config all PaymentSession config options *other* than callbacks
 * @param [forceInitialize] optionally force the whole iframe to reload and rebuild singleton
 * @returns {Promise<HostedSession>}
 */
const configureSession = (paymentSession, config, forceInitialize) => {
  if (sessionInstance && !forceInitialize) {
